...
/* A description of a particular option.  A pointer to an array of
   these is passed in the OPTIONS field of a tiny_argp structure. This should
...
struct tiny_argp_option {
  ...
  /* Name of the argument associated with this option, shown in help output
     ...
     This string is only a display placeholder for help output.  */
  const char *arg;
...
/* Special values for the KEY argument to an argument parsing function.
   ...
   Example: `./foo --verbose bar baz` produces the sequence
   INIT, opt(--verbose), ARG(bar), ARG(baz), END, SUCCESS, FINI.
 ...
 * ## Possible errors
 *
 * Messages the library prints; `<name>` is the program name in `state->name`.
 *
 ...
 *
 * TINY_ARGP_KEY_ARGS:
 *   - return UNKNOWN: SUCCESS -> FINI (END is skipped)
 *   - return SUCCESS: END -> SUCCESS -> FINI (args treated as consumed)
 *
 * ## With NO_EXIT:
 *
...
/* A tiny_argp structure bundles the option declarations, the parser callback
   that handles them, the printer callbacks used for output, and the doc
   strings shown in help output. */
struct tiny_argp {
  ...
  /* Output sink for `--help` / `--usage` text.  Must not be NULL.  The library
     only ever invokes it with `%s` and `%c` conversion specifiers, so any
     minimal printf-style implementation will do.  */
  const tiny_argp_printer_t printer;
  /* Output sink for error messages and usage output produced on bad input.
     Must not be NULL.  Same `%s` / `%c`-only calling convention as
     `printer`.  */
...
struct tiny_argp_state {
  /* The `tiny_argp` being parsed.  */
  const struct tiny_argp *root_tiny_argp;

  /* The number of arguments in `argv`.  */
  size_t argc;
  /* The argument vector being parsed; may be modified.  The library can
     rewrite this array in-place during parsing (e.g. reordering entries so
     that options come first), so callers should treat it as owned by the
     tiny_argp_parse call for its duration.  */
  char **argv;

  /* The index in ARGV of the next arg to be parsed.  May be modified.
  ...
  /* The flags supplied to tiny_argp_parse.  */
  unsigned flags;
  ...
  void *pstate; /* Private, for internal use by tiny_argp.  */
};
 ...
 * If this flag is set, the library will not populate state->name from argv[0];
 * it stays as its default `""`.  Overwrite state->name inside the parse
 * function (typically at KEY_INIT) if you want a meaningful program name to
...
/* Don't provide the built-in `--help` / `--usage` options.
 * Without this flag they are added automatically, and when the user passes
 * either, tiny_argp prints the appropriate output and stops parsing
 * successfully (returning `TINY_ARGP_HELP_OPT`). */
 ...
 * non-NULL, the index in ARGV of the first unparsed arg is returned in it.
 * INPUT is a pointer to a value to be passed in to the parser.
 *
 * If some parser routine returned a non-zero value, it is returned; otherwise 0
 * is returned.
 * In addition, the library may itself return:
 *   - `TINY_ARGP_HELP_OPT` when the built-in `--help` or `--usage` was matched
 *     (unless suppressed by `TINY_ARGP_NO_HELP`);
 *   - `TINY_ARGP_ERR_PARSE` when the library itself detected a bad command
 *     line (unknown option, missing required argument, etc.). */
 ...
 * TINY_ARGP_HELP_*.
 * This function omits the '--help' and `--usage` entries from the option list.
 */
...
/* The following routines are meant to be called from within a parser callback,
 * and so take a tiny_argp_state as their first argument.
 *
 * [tiny_argp_usage & tiny_argp_error should probably be
 * called tiny_argp_state_..., but they're used often enough that they should be
 * short]
 *
 * These functions may or may not print an error message, depending on the flags
 * in STATE.
 * On their own they do NOT abort parsing.
 * If the caller wishes to surface an error and abort parsing the callback must
 * return a non-zero non-TINY_ARGP_ERR_UNKNOWN value, or call tiny_argp_exit
 * (see below).
 * */
...
/* Aborts parsing: after this call, tiny_argp_parse will return the given
 * status code no matter what the parser callback returns next, and no further
 * keys will be dispatched.
 * This is the tiny_argp analogue of calling libc `exit()` from a parser
 * callback, but without actually terminating the runtime, only tiny_argp_parse
 * unwinds. */
